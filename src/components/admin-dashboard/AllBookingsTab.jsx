    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        ...
        <div className="d-flex gap-2 flex-wrap">
          {/* Status pills — hide labels on small screens */}
          <div className="d-flex gap-1 flex-wrap">
            {['All', 'Confirmed', 'Completed', 'Cancelled'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className="border-0 fw-medium"
                style={{ padding: '6px 12px', borderRadius: 20, fontSize: 12, background: statusFilter === s ? '#1a1a1a' : '#f0f0f0', color: statusFilter === s ? '#fff' : '#555' }}>
                {s}
              </button>
            ))}
          </div>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." className="rr-input" style={{ width: 160 }} />
        </div>
      </div>
      ...
      <div className="rr-card overflow-hidden">
        <div className="rr-scrollable-table">
          <table className="table mb-0">
            <thead>
              <tr>{['ID', 'Car', 'User', 'Pickup', 'Drop-off', 'Pickup Date', 'Return Date', 'Amount', 'Status', 'Type'].map(h => <th key={h} className="rr-th">{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map(b => (
                <tr key={b.id}>
                  ...
                  <td className="rr-td">
                    <span className="rr-badge-pill" style={{ background: b.isHourly ? '#fef3ee' : '#f0fdf4', color: b.isHourly ? '#e85d24' : '#16a34a', fontWeight: 500 }}>
                      {b.isHourly ? 'Hourly' : 'Daily'}
                    </span>
                  </td>
                </tr>
              ))}
