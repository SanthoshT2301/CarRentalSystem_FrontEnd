    ...
    <div>
      ...
      {showCarForm && (
        <div className="rr-card mb-4" style={{ padding: 28 }}>
          ...
          {/* Responsive form grid: 3 cols desktop, 2 tablet, 1 mobile */}
          <div className="rr-fleet-form-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {[
              ['make', 'Make (Brand)'], ['model', 'Model'], ['year', 'Year'],
              ['type', 'Type'], ['location', 'Location'], ['pricePerDay', 'Price/Day (₹)'],
              ['noSeats', 'Seats'], ['transmission', 'Transmission'],
              ...
              <div key={k}>
                ...
                {k === 'type' ? (
                  ...
                  <input
                    ...
                    type={['year', 'pricePerDay', 'noSeats'].includes(k) ? 'number' : 'text'}
                    className="rr-input" placeholder={l}
                  />
                )}
              </div>
            ))}
      ...
      {/* Car grid — responsive via rr-fleet-grid class */}
      <div className="rr-fleet-grid">
        {cars.map(car => (
          <div key={car.id} className="rr-card overflow-hidden">
            <div className="rr-fleet-img-wrap">
              <img
                src={car.image} alt={car.make}
              ...
              />
              <span className="rr-badge-pill position-absolute" style={{ top: 10, left: 10, background: car.available ? '#16a34a' : '#dc2626', color: '#fff' }}>
        ...
        {cars.length === 0 && (
          <div className="text-center text-secondary py-5" style={{ gridColumn: '1 / -1' }}>No cars in fleet.</div>
        )}
      </div>
    </div>
  );
