export default function ReportsTab({ reportType, setReportType, dateRange, setDateRange, loadReport, reportData, downloadReport, flash }) {
  return (
    <div>
      ...
      <div className="rr-card d-flex align-items-end flex-wrap gap-3 mb-4" style={{ padding:20 }}>
        <div>
          <label className="rr-label">Report Type</label>
          <select value={reportType} onChange={e => setReportType(e.target.value)} className="rr-input" style={{ width:180 }}>
        ...
        <button onClick={loadReport} className="border-0 fw-semibold text-white" style={{ padding:'9px 22px', background:'#059669', borderRadius:8, fontSize:13 }}>▶ Run</button>
        <button
          onClick={async () => {
        ...
        {reportData.length === 0 ? (
          ...
          <div style={{ overflowX:'auto' }}>
            <table className="table mb-0">
              <thead><tr>{Object.keys(reportData[0]).map(k => <th key={k} className="rr-th">{k.replace(/([A-Z])/g,' $1').trim().toUpperCase()}</th>)}</tr></thead>
              <tbody>
