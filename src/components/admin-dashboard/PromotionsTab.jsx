  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <button onClick={() => setShowPromoForm(true)} className="border-0 fw-semibold text-white" style={{ background:'#7c3aed', padding:'10px 22px', borderRadius:10, fontSize:14 }}>+ Create Promo</button>
      </div>
      ...
      <div className="d-flex flex-column gap-2">
        {promos.map(p => (
          <div key={p.promotionId} className="rr-card d-flex align-items-center gap-3" style={{ padding:'20px 24px' }}>
            <div className="rr-promo-icon d-flex align-items-center justify-content-center" style={{ background: p.active ? '#f3e8ff' : '#f5f5f5' }}>🎟</div>
            ...
            <div className="d-flex gap-2">
              <button onClick={() => handleTogglePromo(p.promotionId)} className="fw-medium" style={{ padding:'7px 16px', border:`1.5px solid ${p.active ? '#fca5a5' : '#bbf7d0'}`, color: p.active ? '#dc2626' : '#16a34a', background:'#fff', borderRadius:8, fontSize:13 }}>
